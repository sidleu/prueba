...
import com.reconosersdk.reconosersdk.optimizedCamera.preference.PreferenceUtils;
...
public class CameraSource {
    ...
    public static final int DEFAULT_REQUESTED_CAMERA_PREVIEW_HEIGHT = 960;
    ...
    private FaceAndTextProcessor frameProcessor;
    /**
    ...
    public void setMachineLearningFrameProcessor(FaceAndTextProcessor processor) {
        synchronized (processorLock) {
            cleanScreen();
            if (frameProcessor != null) {
                frameProcessor.stop();
    ...
    private class FrameProcessingRunnable implements Runnable {
        ...
        @SuppressLint("Assert")
        void release() {
            if (processingThread != null) {assert (processingThread.getState() == Thread.State.TERMINATED);}
        }
     ...
     */
    public void takePicture(com.reconosersdk.reconosersdk.recognition.common.CameraSource.ShutterCallback shutter, com.reconosersdk.reconosersdk.recognition.common.CameraSource.PictureCallback jpeg) {
        synchronized (processorLock) {
            if (camera != null) {
    ...
    private class PictureStartCallback implements Camera.ShutterCallback {
        private com.reconosersdk.reconosersdk.recognition.common.CameraSource.ShutterCallback mDelegate;
    ...
    private class PictureDoneCallback implements Camera.PictureCallback {
        private com.reconosersdk.reconosersdk.recognition.common.CameraSource.PictureCallback mDelegate;

        @Override
        public void onPictureTaken(byte[] data, Camera mCamera) {
            if (mDelegate != null) {
                /*Bitmap bitmap = BitmapFactory.decodeByteArray(data, 0, data.length);
                bitmap = ImageUtils.getSquareImage(bitmap);
                ByteArrayOutputStream blob = new ByteArrayOutputStream();
                bitmap.compress(Bitmap.CompressFormat.WEBP, 100 *//* Ignored for PNGs *//*, blob);
